      ...
      <section id="pricing" className="py-20 px-6 bg-white/[0.01]">
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-10">
            ...
            <p className="text-white/40 text-lg mb-6">No hidden fees. Keep 100% of your royalties. Cancel anytime.</p>
            {/* Billing toggle */}
            <div className="inline-flex items-center gap-1 p-1 bg-white/[0.04] rounded-xl border border-white/[0.06]">
              <span className="px-5 py-2 rounded-lg text-sm font-medium text-white/35">Monthly</span>
              <span className="px-5 py-2 rounded-lg text-sm font-medium bg-white/10 text-white flex items-center gap-2">
                Annual
                <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 font-semibold">Save 40%</span>
              </span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            {/* Musician */}
            <div className="p-6 rounded-2xl border border-white/[0.06] bg-white/[0.02] flex flex-col">
              <span className="text-[10px] font-bold px-2.5 py-1 rounded-full bg-blue-500/20 text-blue-400 border border-blue-500/30 w-fit mb-4">Great Value</span>
              <h3 className="text-xl font-bold text-white mb-1">Musician</h3>
              <div className="flex items-baseline gap-1 mb-1">
                <span className="text-4xl font-bold text-white">$2.08</span>
                <span className="text-white/30">/month</span>
              </div>
              <p className="text-[12px] text-white/30 mb-1">$24.99 billed annually</p>
              <p className="text-sm text-white/50 font-medium mb-5">1 artist</p>
              <ul className="space-y-2.5 flex-1 mb-6">
                {["Upload unlimited songs", "All major platforms", "Keep 100% royalties", "Lyrics in Google & more", "Spotify Registered Artist label", "Create royalty splits", "Mobile app access"].map((f) => (
              ...
              <button onClick={onGetStarted} className="w-full py-3 rounded-xl border border-white/10 hover:bg-white/5 text-sm font-bold text-white transition-all">
                Try WaveTrack
              </button>
            </div>

            {/* Musician Plus - highlighted */}
            <div className="p-6 rounded-2xl border-2 border-emerald-500/50 bg-gradient-to-b from-emerald-500/10 to-transparent flex flex-col relative">
              <span className="text-[10px] font-bold px-2.5 py-1 rounded-full bg-emerald-500 text-white w-fit mb-4">Most Popular</span>
              <h3 className="text-xl font-bold text-white mb-1">Musician Plus</h3>
              <div className="flex items-baseline gap-1 mb-1">
                <span className="text-4xl font-bold text-white">$3.75</span>
                <span className="text-white/30">/month</span>
              </div>
              <p className="text-[12px] text-white/30 mb-1">$44.99 billed annually</p>
              <p className="text-sm text-white/50 font-medium mb-1">2 artists</p>
              <p className="text-[11px] text-emerald-400 font-semibold mb-5">Save 15%</p>
              <ul className="space-y-2.5 flex-1 mb-6">
                {["Everything in Musician", "Synced lyrics in Apple Music", "Daily streaming stats", "Customizable label name", "Customizable release date", "Customizable preorder date", "Custom iTunes pricing", "Mobile app access"].map((f) => (
              ...
              <button onClick={onGetStarted} className="w-full py-3 rounded-xl bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-400 hover:to-teal-400 text-sm font-bold text-white transition-all active:scale-95 shadow-lg shadow-emerald-500/25">
                Try WaveTrack
              </button>
            </div>
            ...
            {/* Ultimate */}
            <div className="p-6 rounded-2xl border border-white/[0.06] bg-white/[0.02] flex flex-col">
              <span className="text-[10px] font-bold px-2.5 py-1 rounded-full bg-purple-500/20 text-purple-400 border border-purple-500/30 w-fit mb-4">Save up to 40%</span>
              <h3 className="text-xl font-bold text-white mb-1">Ultimate</h3>
              <div className="flex items-baseline gap-1 mb-1">
                <span className="text-4xl font-bold text-white">$7.50</span>
                <span className="text-white/30">/month</span>
              </div>
              <p className="text-[12px] text-white/30 mb-1">$89.99 billed annually</p>
              <p className="text-sm text-white/50 font-medium mb-5">5+ artists</p>
              <ul className="space-y-2.5 flex-1 mb-6">
                {["Everything in Musician Plus", "Free iPhone app access", "1TB instant file sharing", "Playlist contact database", "Priority distribution", "Advanced analytics"].map((f) => (
                  <li key={f} className="flex items-start gap-2.5 text-[13px] text-white/55">
                    <svg width="14" height="14" fill="none" stroke="#34d399" strokeWidth="2.5" strokeLinecap="round" className="flex-shrink-0 mt-0.5"><polyline points="2,7 5,10 12,3"/></svg>
                    {f}
                  </li>
          ...
          <p className="text-center text-[12px] text-white/20 mt-6">
            Cancel anytime · 7-day free trial · No credit card required · Secure payment via Stripe
          </p>
        </div>
      </section>
