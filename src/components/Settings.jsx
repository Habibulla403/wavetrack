    ...
    <div className="space-y-6">
      ...
      <div className="rounded-2xl border border-white/[0.06] bg-white/[0.02] p-6">
        ...
        <div className="flex items-center justify-between flex-wrap gap-4">
          ...
          {!isPro && (
            <button className="px-5 py-2.5 rounded-xl bg-gradient-to-r from-emerald-500 to-teal-500 text-sm font-bold text-white shadow-lg shadow-emerald-500/20">
              ✨ Upgrade to Pro
            </button>
          )}
        </div>
      ...
      <div className="rounded-2xl border border-red-500/20 bg-red-500/5 p-6 space-y-4">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          ...
          <button onClick={onLogout} className="px-5 py-2 rounded-xl border border-white/10 text-white/50 hover:bg-white/5 text-sm font-medium transition-all">
            Logout
          </button>
        </div>
        <div className="border-t border-red-500/10 pt-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          ...
          {!showDeleteConfirm ? (
            ...
            <div className="flex gap-2">
              <button className="px-4 py-2 rounded-xl bg-red-500 text-sm font-semibold text-white">Confirm Delete</button>
              <button onClick={() => setShowDeleteConfirm(false)} className="px-4 py-2 rounded-xl bg-white/5 text-sm text-white/50">Cancel</button>
            </div>
          )}
        </div>
      </div>
